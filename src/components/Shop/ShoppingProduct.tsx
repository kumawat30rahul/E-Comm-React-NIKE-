...
function ShoppingProduct() {
  const { filterArrays, category, filterData, setFilterData, data, setData } =
  ...
  const filterFunction = () => {
    ...
    if (filterArrays.length === 0) {
      ...
      const sizeFilters = filterArrays.filter(
        (filter: any) => filter.length <= 3 && !filter.includes("-")
      );
      ...
      const priceFilters = filterArrays.filter(
        (filter: any) => filter.length > 3 && filter.includes("$")
      );
        ...
        filteredProduts = filteredProduts.filter((product: any) => {
          return priceFilters.some((range: any) => {
            const [minPrice, maxPrice] = range
              .split("-")
              .map((price: any) => parseFloat(price.replace("$", "")));
      ...
      const ratingFilter = filterArrays.filter(
        (filter: any) => filter.length === 3 && filter.includes("-")
      );
